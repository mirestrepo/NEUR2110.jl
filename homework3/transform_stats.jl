...
abstract type TransformStatistic{T<:Real} end
abstract type PairwiseTransformStatistic{T<:Real} <: TransformStatistic{T} end
...
function init{T}(s::PairwiseTransformStatistic{T}, nout, nchannels, ntapers)
    ...
    s.x = zeros(Float64, nout, size(s.pairs, 2))
...
accum(s::PairwiseTransformStatistic, fftout, itaper) =
...
function accum(s::PowerSpectrum, fftout, itaper)
    A = s.x
...
function accum(s::PowerSpectrumVariance, fftout, itaper)
    @inbounds begin
        A = s.x
...
function accum(s::ShiftPredictor, fftout, itaper)
